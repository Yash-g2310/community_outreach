...
/// Rider's active-ride screen backed solely by FastAPI snapshots and events.
class UserTrackingPage extends StatefulWidget {
  const UserTrackingPage({super.key, required this.rideId});
...
class _UserTrackingPageState extends State<UserTrackingPage> with SafeStateMixin {
  ...
  Future<void> _initialize() async {
    ...
    if (!_isTerminal(_status)) {
      await _locationPublisher.start(
        ...
        onLocation: (location) => safeSetState(() => _riderLocation = location),
      );
    }
  ...
  Map<dynamic, dynamic> _decode(String body) {
  ...
  Future<void> _handleTerminalState(String? reason) async {
    ...
    if (mounted) AppRouter.pushReplacement(context, const UserMapScreen());
  }

  Future<void> _cancelRide() async {
    if (_isCancelling || _isTerminal(_status)) return;
    ...
    try {
      ...
      if (mounted) safeSetState(() => _isCancelling = false);
    }
  }
  ...
  String _terminalMessage(String status) {
    switch (status) {
      case 'completed':
        return 'The driver marked this ride as completed.';
  ...
  Widget build(BuildContext context) {
    final center = _riderLocation ?? _driverLocation;
    return PopScope(
