...
class WebSocketService {
  ...
  Future<void> reconnectWithAccessToken(String accessToken) async {
    ...
    if (reconnectPassenger) {
      ...
      await connectPassenger(jwtToken: accessToken);
    }
    if (reconnectDriver) {
      await _replaceDriverSocket();
      await connectDriver(jwtToken: accessToken);
    }
  }
  ...
  void _schedulePassengerReconnect() {
    ...
    _passengerReconnectTimer = Timer(delay, () async {
      ...
      await connectPassenger(jwtToken: _passengerJwtToken);
    });
  }
  ...
  void _scheduleDriverReconnect() {
    ...
    _driverReconnectTimer = Timer(delay, () async {
      ...
      // Attempt reconnection with stored tokens
      await connectDriver(jwtToken: _driverJwtToken);
    });
  }
